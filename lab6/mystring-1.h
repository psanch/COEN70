...
//   It is originally part of the namespace main_savitch_4, from the textbook
...
//     Postcondition: If n == allocated, do nothing. If n > current_length, an n byte array
//     is allocated and the current value of sequence is stored in that new memory. If n <
//     current_length, allocate an array of size current_length + 1.
//
...
//INVARIANT: Valid string objects are represented by a dynamic array size allocated. Terminated with '\0' character. Current_length stores the amount of bytes currently occupied by char's. 

#ifndef COEN70_LAB5_STRING_H
#define COEN70_LAB5_STRING_H
...
namespace coen70_lab
{
    ...
    public:
        ...
        void insert(const string& source, unsigned int position);   // NEW FUNCTION 2
        void dlt(unsigned int position, unsigned int num);          // NEW FUNCTION 3
        void replace(char c, unsigned int position);                // NEW FUNCTION 4
        void replace(const string& source, unsigned int position);  // NEW FUNCTION 5
        // CONSTANT MEMBER FUNCTIONS
        size_t length( ) const { return current_length; }
        char operator [ ](size_t position) const;
        int search(char c) const;                                   // NEW FUNCTION 6
        int search(const string& substring) const;                  // NEW FUNCTION 8
        unsigned int count(char c) const;                           // NEW FUNCTION 7
        // FRIEND FUNCTIONS
